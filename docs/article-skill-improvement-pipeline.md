# Skill 结构评分与执行效果零相关：我们怎么重新设计评估和自动优化

> 给 28 个 Skill 跑结构评分，然后用真实任务验证——发现两者之间 R²=0.00。
> 基于这个发现，重新搭了一套从执行评估到自动改进到用户反馈闭环的完整管线。15 个 skill、8000 行 Python、432 个测试，已发布到 ClawHub。

## Skill 质量的度量问题

OpenClaw 生态的 Skill 数量已经上万（VoltAgent 精选集筛选后 5000+），团队内部也有几十个。但一个被回避的问题是：**怎么客观判断一个 Skill 的质量？**

目前的做法：手动试几次、让同事跑几个 case、看看能不能触发。Skill 少的时候能凑合，规模上来之后就不够了——没有统一标准，没有稳定性度量，更没有自动化的改进闭环。
