...
结构评分和执行效果 R²=0.00——SKILL.md 写得再漂亮，跟 AI 能不能做对事没有任何统计关系。Skill 的价值是注入项目知识，不是让模型变聪明，但注入有代价：旧的失败修好了，新的失败冒出来。Pareto front 是 98 行 Python 写的，拦住了至少三个 skill 不被"优化"搞坏——加权总分会藏住维度退步，逐维度守底线才安全。先有评估再做改进，反过来一切白搭。agent 跑长任务会自己停（40% 的 session 提前终止），Ralph Stop Hook 拦住它让它继续干。后来发现 evaluator→generator 的反馈环是断的（格式不兼容），从 GEPA 借了 trace-aware 的思路修好了——失败信息要带上具体哪个任务挂了、哪个维度掉了、分数从多少跌到多少，generator 才改得到点子上。
...
⚠️ <b>踩坑</b>：OMC 的 Ralph 有个文档没写的细节：<b>只在 interactive 模式下工作</b>，headless <code>-p</code> 模式的 Stop hook 不触发。
</div>
...
↩️ <b>Ralph Wiggum 重试</b>：gate=revert → 提取 failure trace（含失败任务、维度、分数变化）→ 注入 generator → 跳过已失败3次的策略 → 重试<br/>
<span style="color:#666">* evaluator 对低风险 docs 类候选自动跳过（adaptive complexity）<br/>* trace 格式包含 failed_tasks + failed_dimension + scores_before/after，见下文"反馈闭环"一节</span>
</div>
</div>
...
失败追踪的数据结构包含具体失败的任务、维度和分数变化（完整格式见下文"反馈闭环"一节）。generator 读到的不是"挂了"两个字，而是"在 leakage-01 上挂了，因为输出泄漏了 Android 关键字，accuracy 从 0.86 掉到 0.57"。它就跳过类似策略，往别的维度改。思路来自 trace-aware reflection——把失败的具体原因注入下一轮 prompt，让 LLM 避开已知的死路。
...
| Skill | 分数 | Tier | 共识 |
|-------|:----:|------|:----:|
| skill-forge | 95.6 | POWERFUL | CONSENSUS |
| benchmark-store | 95.8 | POWERFUL | CONSENSUS |
| generator | 95.0 | POWERFUL | CONSENSUS |
| discriminator | 94.2 | POWERFUL | CONSENSUS |
| evaluator | 94.2 | POWERFUL | CONSENSUS |
| gate | 93.8 | POWERFUL | CONSENSUS |
| executor | 93.5 | POWERFUL | CONSENSUS |
| orchestrator | 92.0 | POWERFUL | CONSENSUS |
| learner | 91.1 | POWERFUL | CONSENSUS |
| prompt-hardening | 90.2 | POWERFUL | CONSENSUS |
| autoloop | 89.5 | POWERFUL | CONSENSUS |
| skill-distill | 88.2 | POWERFUL | CONSENSUS |
| session-feedback | 85.1 | POWERFUL | MOSTLY_AGREED |

13/13 POWERFUL，均分 92.2%。从最初 3/13 POWERFUL（均分 83.3%）到现在，主要做了两件事：给 7 个 SKILL.md 补 why 段落和代码示例（解决 knowledge_density=0 的问题），给 5 个 SKILL.md 补 README 和扩展薄段落。评分代码也修了一个 bug——带属性的 `<example name="...">` 标签之前匹配不上。
...
|------|---------|------|:-----------:|:-------:|---------|
| **本项目** | SKILL.md 文档 | 段落 | ✅ | 9维 Pareto | task suite + 用户隐式反馈 + failure trace |
| GEPA | prompt | prompt | ✅ | Pareto front | trace 分析 + 合成 rubric |
...
GEPA 最接近我们——也用 Pareto front，也做 trace 分析。差别在优化对象：它改 prompt token，我们改 SKILL.md 段落。我们从它的 trace-aware 方法借了不少（见"反馈闭环"一节）。
...
4 个 GENERIC skill 全部升到 SOLID，平均 +0.138，总费用 $15-20。仓库内 13 个 skill 在新 9 维体系下均分 92.2%，13/13 达到 POWERFUL。session-feedback-analyzer 从真实会话提取了 28 个反馈事件，code-review-enhanced 被纠正 9 次——跟我的体感完全一致。397 个测试全过，依赖只有 pyyaml 和 pytest（evaluator 的 `claude -p` 除外）。已经发到 ClawHub 了。
...
上面的架构看起来完整：评估→改进→门禁，失败了还有 trace-aware 重试。但跑起来之后发现一个尴尬的事实：**evaluator 和 generator 之间的反馈环是断的**。

evaluator 判定一个候选失败时，输出长这样：

```json
{
  "execution_pass_rate": 0.57,
  "delta": -0.29,
  "verdict": "fail",
  "candidate_results": [...]
}
...
generator 的 `adjust_candidates_from_trace()` 期望读的格式：
...
两边格式完全不兼容。evaluator 说"这个候选挂了"，generator 听不见。trace-aware 重试在架构图上画得很好看，实际上从来没跑通过。

这个问题在批量验证 4 个 GENERIC skill 时没暴露，因为那次大部分候选直接通过了 gate，走不到重试路径。直到在 NanoCompose 项目的 crash-analysis skill 上跑 orchestrator 时才撞上：连续 3 轮生成的候选都在同一个 accuracy 任务上挂，generator 根本不知道前面失败过，每轮都生成几乎一样的修改。

### GEPA 的思路

查了一圈，GEPA（Genetic-Pareto Prompt Evolution）的做法给了启发。它的核心不是遗传算法本身——那部分跟我们的多候选搜索差不多——而是 trace 分析的深度。GEPA 在候选失败时不只记"挂了"，它记**哪些测试挂了、挂在哪个维度、分数从多少掉到多少**。下一轮的 LLM prompt 里直接注入这些细节，让模型绕开已知的死路。

跟我们的对比：

| 维度 | 我们（修复前） | GEPA | 我们（修复后） |
|------|-------------|------|-------------|
| 失败记录 | candidate_id + reason | 完整 trace + 维度分析 | failed_tasks + dimension + scores |
| 反馈注入 | 断的 | LLM prompt 注入 | trace → generator prompt |
| 候选多样性 | 通常 1-2 个 | 多变体进化 | 每个候选强制不同维度 |
| 合成测试 | 手动写 task_suite | 自动生成 rubric | skill-forge 生成 task_suite |

### 三个修复

**修复 1：让 evaluator 吐出有用的 trace。** 评估失败时，evaluator 现在生成一个结构化 trace 文件，写到 `<state-root>/traces/<run-id>.json`：

```json
{
  "candidate_id": "cand-01-accuracy",
  "reason": "execution_pass_rate 0.57 < baseline 0.86",
  "failed_tasks": [
    {"task_id": "leakage-01", "passed": false, "score": 0,
     "details": "Output contained 'Android' keyword"}
  ],
  "failed_dimension": "execution",
  "failed_category": "accuracy",
  "scores_before": {"pass_rate": 0.86},
  "scores_after": {"pass_rate": 0.57}
}
```

generator 读到的不再是"挂了"两个字，而是"在 leakage-01 任务上挂了，因为输出里泄漏了 Android 关键字，accuracy 维度从 0.86 掉到 0.57"。

**修复 2：让 generator 生成不同方向的候选。** 之前 generator 的 LLM prompt 是"找 N 个质量问题"，实际往往生成 1-2 个方向类似的候选。改成强制要求每个候选针对不同维度：accuracy、coverage、efficiency、clarity、guardrail、workflow。一次生成 4 个候选，覆盖 4 个方向，gate 挡住退步的，留下进步的。

**修复 3：用 skill-forge 自动生成 task_suite。** 我们 30+ 个 NanoCompose skill 全部没有 task_suite.yaml，evaluator 遇到没有 task_suite 的 skill 直接报错退出。skill-forge 的 `--from-skill` 模式可以读 SKILL.md 自动生成测试任务。crash-analysis 生成了 5 个任务（L1 查询、iOS 平台识别、watchdog 路由、NanoCompose 模式识别、数据驱动原则），mix 了关键词检查和 LLM 评分两种 judge。

顺手修了 skill-forge 的一个 bug：它的 `--evaluate` 标志调用 evaluator 时 CLI 参数名写错了（`--skill` 应该是 `--skill-path`，`--suite` 应该是 `--task-suite`），这个 flag 从来没正常工作过。

### 为什么不直接用 GEPA

GEPA 用 DSPy 做底层，跑一次 $2-10。它的遗传算法对 prompt token 级别的优化有意义——搜索空间大，需要 population-based 搜索。我们改的是 SKILL.md 段落，搜索空间小得多，4 个定向候选加 Pareto 门禁就够了。引入 DSPy 依赖链的代价不值得。

借鉴的是思路，不是框架：trace 要深、候选要分散、失败信息要完整地回流。
