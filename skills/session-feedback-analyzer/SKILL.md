...
- Rebuild a noisy `feedback.jsonl` with `--overwrite` after improving the analyzer rules.
- Export markdown hotspot summaries for QMD or study-brain ingestion via `scripts/export_feedback_bridge.py`.
...
```bash
python3 scripts/analyze.py --session-dir ~/.claude/projects/ --output feedback-store/feedback.jsonl --overwrite
python3 scripts/export_feedback_bridge.py --input feedback-store/feedback.jsonl --output feedback-store/$(date +%F)-session-feedback-hotspots.md
```
...
# Rebuild after classifier fixes
...
| `--overwrite` | off | Rebuild the destination file instead of append+dedupe |
...
- `feedback-store/YYYY-MM-DD-session-feedback-hotspots.md` -- markdown bridge output for QMD/study-brain ingestion, generated by `scripts/export_feedback_bridge.py`.
...
- `observer-sessions/` — memory observer / synthetic relay sessions，不应污染真实 skill 反馈

Auto-archives events >90 days old to `feedback-store/archive/`
to keep the active feedback store small and fast to query。

Slash command detection also validates that the command maps to a real installed skill. Builtins such as `/read` and `/exit` are ignored instead of being misattributed as skill invocations.
...
| `scripts/export_feedback_bridge.py` | Convert feedback.jsonl into markdown hotspot lessons for QMD/study-brain |
| `scripts/metrics.py` | Metrics library: correction_rate, trend, hotspots, report formatting |
| `tests/test_analyze.py` | Invocation detection, observer filtering, overwrite rebuild, and classification coverage |
